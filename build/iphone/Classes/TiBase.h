...
// in simulator we redefine to format for MUMAS Developer console
...
{\
	NSString * exceptionName = [@"org.mumas." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.mumas";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	TiExceptionThrowWithNameAndReason(exceptionName,message);\
}\
...
enum {
	UIMUMASNativeItemNone = -1, 
	UIMUMASNativeItemSpinner = -2,
	UIMUMASNativeItemProgressBar = -3,
	
	UIMUMASNativeItemSlider = -4,
	UIMUMASNativeItemSwitch = -5,
	UIMUMASNativeItemMultiButton = -6,
	UIMUMASNativeItemSegmented = -7,
	
	UIMUMASNativeItemTextView = -8,
	UIMUMASNativeItemTextField = -9,
	UIMUMASNativeItemSearchBar = -10,
	
	UIMUMASNativeItemPicker = -11,
	UIMUMASNativeItemDatePicker = -12,
	
	UIMUMASNativeItemInfoLight = -13,
	UIMUMASNativeItemInfoDark = -14,
	
	UIMUMASNativeItemDisclosure = -15,
	
	UIMUMASNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all MUMAS instances of
